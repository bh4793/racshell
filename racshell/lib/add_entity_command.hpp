...
inline int run_add_entity_command(int argc, char* argv[], const AddEntityCommandSpec& spec) {
    ...
    std::string entity_value = program.get<std::string>(spec.entity_argument);
    ...
    SearOperationInfo op_info = validate_sear_operation_result(result->result_json);
    if (!op_info.success) {
    ...
    std::cout << spec.success_label << " " << entity_value << " created successfully.\n";
    return 0;
}
