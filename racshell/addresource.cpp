...
{
    argparse::ArgumentParser program("addresource");

    program.add_argument("resource")
        .help("resource profile name to create, e.g. IRR.RADMIN.**");

    program.add_argument("class")
        .help("resource profile class, e.g. facility");

    program.add_argument("-t", "--traits")
        .help("traits to set, e.g. base:universal_access=None base:owner=SECADM")
    ...
    std::cout << "Resource profile " << resource << " created successfully in class " << resource_class << ".\n";
    return 0;
}
