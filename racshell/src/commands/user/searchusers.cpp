...
{
    argparse::ArgumentParser program("searchusers");

    program.add_argument("-f", "--filter")
        .help("filter user profiles by userid pattern, e.g. IBM")
    ...
    if (response.contains("profiles") && response["profiles"].is_array())
    {
        for (const auto &profile : response["profiles"])
        {
            ...
            {
                users.push_back(profile.get<std::string>());
            }
        }
    }
